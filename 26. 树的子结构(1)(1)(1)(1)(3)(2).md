...
1. **全局变量：**有全局作用域，**只需要在一个源文件中定义即可作用域所有的源文件**(其他不含全局变量定义的源文件需要用**extern**关键字再次声明这个全局变量)
...
​	当该头文件被多个文件include时，该头文件的全部变量会被定义多次，导致重复定义。
...
​		将构造函数设置为protected，然后在public里提供一个静态的create函数，在create里使用new来完成构造，而不是在类的外部使用new构造。另外，析构函数设为protected。
...
1.作用于局部变量： static修饰的局部变量，在初始化之后直到程序运行结束才销毁。**此外，static修饰的局部变量只会被初始化一次**。
